			...
			if (tokenIsValid) {
				_data.list('items', (err, itemList) => {
					if (! err && itemList && itemList.length > 0) {
						var result = [];
						var errors = false;
						itemList.forEach((fileName, index, array) => {
							...
							_data.read('items', fileName, (err, itemData) => {
								...
								// When we encounter the last element of the array
											...
											_data.delete('carts', emailAddress, err => {
												if (! err) {
													...
													const charge = stripe.charges.create({
														...
														if (! err) {
															handlers._order.sendMailReceipt(emailAddress, receiptText, statusCode => {
																if ([200, 201, 202, 203].indexOf(statusCode) > -1) {
																	callback(200);
																} else {
...
handlers._order.sendMailReceipt = (emailAddress, receiptText, callback) => {
