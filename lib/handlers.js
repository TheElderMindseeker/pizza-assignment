...
handlers.index = (data, callback) => {
	...
	if (data.method == 'get') {
		...
		var templateData = {
			'head.title': 'Pizza Delivery - Hot and Tasty',
			'head.description': 'Using our service, you can order a delicious pizza right to your home without any problems',
			'body.class': 'index'
		};
		...
		helpers.getTemplate('index', templateData, (err, str) => {
			if (! err && str) {
				helpers.addUniversalTemplates(str, templateData, (err, fullString) => {
					if (! err && fullString && fullString.length > 0) {
...
handlers.menuList = (data, callback) => {
	...
	if (data.method == 'get') {
		...
		// Read in the index template as a string
		helpers.getTemplate('menuList', templateData, (err, str) => {
			if (! err && str) {
				helpers.addUniversalTemplates(str, templateData, (err, fullString) => {
					if (! err && fullString && fullString.length > 0) {
						callback(200, fullString, 'html');
...
handlers.cartList = (data, callback) => {
	...
	if (data.method == 'get') {
		// Prepare data for interpolation
		var templateData = {
			'head.title': 'User Cart',
			'body.class': 'cartList'
		};

		// Read in the index template as a string
		helpers.getTemplate('cartList', templateData, (err, str) => {
			if (! err && str) {
				helpers.addUniversalTemplates(str, templateData, (err, fullString) => {
					if (! err && fullString && fullString.length > 0) {
						callback(200, fullString, 'html');
			...
			if (! err && itemList && itemList.length > 0) {
				...
				itemList.forEach(fileName => {
					_data.read('items', fileName, (err, itemData) => {
						if (! err && itemData) {
							templateData['form.fields'] += `<div class="inputWrapper">\n`;
							templateData['form.fields'] += `<div class="inputLabel">${itemData.title}</div>\n`
							templateData['form.fields'] += `<input type="number" class="itemInput" id="${itemData.id}" placeholder="Amount" name="${fileName}">\n`;
							templateData['form.fields'] += '</div>\n';
						...
						if (itemsReadCount == itemList.length) {
							...
							if (result.length > 0) {
								// Read in the index template as a string
								helpers.getTemplate('cartCreate', templateData, (err, str) => {
									if (! err && str) {
										helpers.addUniversalTemplates(str, templateData, (err, fullString) => {
											if (! err && fullString && fullString.length > 0) {
												callback(200, fullString, 'html');
...
handlers._cart.post = (data, callback) => {
	...
	if (order.length === 0) {
		...
		_data.list('items', (err, itemList) => {
			if (! err && itemList && itemList.length > 0) {
				itemList.forEach(fileName => {
					var itemAmount = typeof(data.payload[fileName]) == 'string'
						&& data.payload[fileName] > 0
						&& data.payload[fileName] % 1 === 0
						? parseInt(data.payload[fileName]) : 0;
							...
							handlers._tokens.verifyToken(token, emailAddress, tokenIsValid => {
								if (tokenIsValid) {
									_data.read('carts', emailAddress, (err, cartData) => {
										// Either create new file or replace the old one
										if (err) {
											var writeMethod = _data.create;
										} else {
											var writeMethod = _data.update;
										}
										...
										_data.list('items', (err, itemList) => {
											if (! err) {
												...
												if (cartData.orderList.length > 0) {
													writeMethod('carts', emailAddress, cartData, err => {
														if (! err) {
															callback(200);
														} else {
...
handlers._cart.put = (data, callback) => {
	...
	if (! order) {
		...
		_data.list('items', (err, itemList) => {
			if (! err && itemList && itemList.length > 0) {
				itemList.forEach(fileName => {
					var itemAmount = typeof(data.payload[fileName]) == 'number'
						&& data.payload[fileName] > 0
						&& data.payload[fileName] % 1 === 0
						? data.payload[fileName] : 0;
		...
		order = order.length > 0 ? order : false;
	}
			...
			if (tokenIsValid) {
				...
				_data.read('carts', emailAddress, (err, cartData) => {
					if (! err && cartData) {
						...
						// Create an order object to log after order is completed
						var orderObject = {
						...
						cartData.orderList.forEach((itemOrder, index, array) => {
							_data.read('items', itemOrder.id, (err, itemData) => {
								if (! err && itemData) {
									...
									// Add item's info to orderList
									orderObject.orderList.push({
										"title": itemData.title,
										"price": itemData.price,
											...
											_data.delete('carts', emailAddress, err => {
												if (! err) {
													...
													helpers.createStripeCharge({
														...
														if (! err) {
															_data.create('orders', `${emailAddress}-${orderObject.timestamp}`, orderObject, err => {});
						...
						callback(400, { 'Error': "The user has empty shopping cart" });
					}
				});
			} else {
				callback(403, { 'Error': 'Missing required token in header, or token is invalid' });
